 ...
 */
function unl_fourone_block_view_alter(&$data, $block) {
 ...
 */
function _unl_fourone_block_view_system_main_menu_alter(&$data, $block) {
  $current_menu_link = _unl_fourone_get_current_menu_link();
  if ($current_menu_link) {
    $submenu = _unl_fourone_get_current_submenu($data['content'], $current_menu_link->mlid);
    if (!theme_get_setting('disable_drill_down') && $submenu && $submenu['#original_link']['depth'] > 1) {
      $data['content'] = $submenu['#below'];
    }
  }
  $data['content'] = _unl_fourone_limit_menu_depth($data['content'], 2);
}
 ...
 */
function _unl_fourone_get_current_menu_link() {
 ...
 */
function _unl_fourone_get_current_submenu($menu_links, $current_mlid) {
  foreach (element_children($menu_links) as $index) {
    $menu_item = $menu_links[$index];
    if ($menu_item['#original_link']['mlid'] == $current_mlid) {
      return $menu_item;
    }
    $sub_menu = _unl_fourone_get_current_submenu($menu_item['#below'], $current_mlid);
    if ($sub_menu) {
      return $sub_menu;
    }
  }
 ...
 */
function _unl_fourone_limit_menu_depth($menu_links, $depth) {
  if ($depth == 0) {
    return array();
  }

  foreach (element_children($menu_links) as $index) {
    $menu_links[$index]['#below'] = _unl_fourone_limit_menu_depth($menu_links[$index]['#below'], $depth - 1);
  }
 ...
 */
function unl_fourone_css_alter(&$css) {
  if (!theme_get_setting('unl_affiliate')) {
    unset($css[drupal_get_path('theme', 'unl_fourone') . '/css/colors.css']);
  }
}
 ...
 */
function unl_fourone_preprocess_block(&$vars) {
 ...
 */
function unl_fourone_preprocess_field(&$vars, $hook) {
 ...
 */
function unl_fourone_preprocess_image(&$vars) {
 ...
 */
function unl_fourone_preprocess_html(&$vars, $hook) {
  // Add the CSS and JS files that are generated from the unl_fourone appearance settings page
  foreach (array('css', 'js') as $type) {
    $file = variable_get('unl_custom_code_path', 'public://custom') . '/custom_unl_fourone.' . $type;
  ...
  // Affiliate Template
  if (theme_get_setting('unl_affiliate')) {
    drupal_add_css(drupal_get_path('module', 'unl_fourone') . '/css/colors.css', array('type' => 'file', 'group' => CSS_THEME, 'every_page' => TRUE));
 ...
 */
function unl_fourone_process_html(&$vars) {
 ...
 */
function unl_fourone_preprocess_node(&$vars) {
  ...
  if ($vars['type'] == 'webform') {
    $path = drupal_get_path('theme', 'unl_fourone');
 ...
 */
function unl_fourone_preprocess_username(&$vars) {
 ...
 */
function unl_fourone_username_alter(&$name, $account) {
  if ($account->uid) {
 ...
 */
function unl_fourone_process_page(&$vars) {
  // Hook into color.module.
  if (theme_get_setting('unl_affiliate') && module_exists('color')) {
 ...
 */
function unl_fourone_get_instance() {
  static $instance;
  if (!$instance) {
 ...
 */
function unl_fourone_breadcrumb($variables) {
  $breadcrumbs = $variables['breadcrumb'];

  if (count($breadcrumbs) == 0) {
    $breadcrumbs[] = '<a href="">' . check_plain(unl_fourone_get_site_name_abbreviated()) . '</a>';
  }
  else {
 ...
 */
function unl_fourone_file_icon($variables) {
  return '';
}

function unl_fourone_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  if ($extra_class) {
 ...
 */
function unl_fourone_menu_tree($variables) {
  $tree = $variables['tree'];
 ...
 */
function unl_fourone_get_site_name_abbreviated() {
